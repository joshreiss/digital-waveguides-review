...
class ClarinetModel {
  constructor(context) {
    ...
    this.stereoPanLeft = context.createStereoPanner();
    this.stereoPanRight = context.createStereoPanner();
    ...
    this.parameters = {
      freq: 440, gain: 1.0,
      reedStiffness: 0.5, noiseGain: 0.0,
      pressure: 1.0, nonLinearity: 0.0,
      nonLinAttack: 0.1, vibratoFreq: 5.0,
      vibratoGain: 0.1,  vibratoAttack: 0.5,
      vibratoRelease: 0.01, envelopeAttack: 0.01,
      envelopeDecay: 0.05, envelopeRelease: 0.1
    };    
  ...
  setFrequency(frequency) {
    ...
    this.delayLine.delayTime.setTargetAtTime(
      delayLength / context.sampleRate,
      context.currentTime,
      0.01
    )    
    // Update stereo width based on frequency
    const stereoWidth = Math.min(0.5, 100 / frequency);
    this.stereoPanLeft.pan.setTargetAtTime(-stereoWidth, context.currentTime, 0.05);
    this.stereoPanRight.pan.setTargetAtTime(stereoWidth, context.currentTime, 0.05);
  }  
...
class SimpleReverb {
  constructor(context) {
    ...
    for (let i = 0; i < allpassTimes.length; i++) {
      ...
      const gainInv = context.createGain();
