...
AsmFunctionsWrapper.prototype.pluck = function(channelBuffer,seedNoise,sampleRate,hz,smoothingFactor,velocity,options) {
    ...
    asm.renderKarplusStrong(heapOffsets.seedStart,heapOffsets.seedEnd,heapOffsets.targetStart,
        ...
        options.pluckDamping,options.pluckDampingVariation);
...
function getNextValidFloat32HeapLength(desiredLengthFloats) {
    var heapLengthBytes;
    var desiredLengthBytes = desiredLengthFloats * 4;
...
function asmFunctions(stdlib, foreign, heapBuffer) {// standard asm.js block
    "use asm";
    // heap is supposed to come in as just an ArrayBuffer so first need to get a Float32 view of it
    var heap = new stdlib.Float32Array(heapBuffer);// stdlib: object through which standard library functions called
    var floor = stdlib.Math.floor;// foreign: object through which external javascript functions called
    ...
    function renderKarplusStrong(seedNoiseStart,seedNoiseEnd,targetArrayStart,targetArrayEnd,sampleRate, hz, velocity,
        ...
        for (targetIndex = 0; (targetIndex) < (sampleCount); targetIndex = (targetIndex + 1)) {
            heapTargetIndexBytes = (targetArrayStart + targetIndex) * 4;
            if ((targetIndex) < (periodSamples)) {
                // for the first period, feed in noise. remember, heap index has to be bytes...
                heapNoiseIndexBytes = (seedNoiseStart + targetIndex) * 4;
                ...
                lastPeriodStartIndexBytes = (heapTargetIndexBytes - (periodSamples * 4));
                skipSamplesFromTension =  ~~floor(stringTension * (+(periodSamples>>>0)));
                lastPeriodInputIndexBytes = (lastPeriodStartIndexBytes + (skipSamplesFromTension * 4));
