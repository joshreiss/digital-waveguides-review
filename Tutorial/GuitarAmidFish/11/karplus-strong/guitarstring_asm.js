...
AsmFunctionsWrapper.prototype.pluckSine = function(channelBuffer, sampleRate,hz, velocity, decayFactor) {
    ...
    var heapOffsets = { targetStart: 0, targetEnd: channelBuffer.length-1 }
...
AsmFunctionsWrapper.prototype.pluck = function(channelBuffer,seedNoise,sampleRate,hz,
    smoothingFactor,velocity,options,acousticLocation) {
    ...
    var targetArrayL = channelBuffer.getChannelData(0);
    var targetArrayR = channelBuffer.getChannelData(1);
    // string.acousticLocation is set individually for each string so that lowest note has value of -1 and the highest +1
...
function asmFunctions(stdlib, foreign, heapBuffer) {
    ...
    // simple discrete-time lowpass filter from Wikipedia
    function lowPass(lastOutput, currentInput, smoothingFactor) {
        // coersion to indicate type of arguments
        // +x represents double
        // we do the arithmetic using doubles rather than floats, because in asm.js spec, operations done floats 
        // resolve to 'floatish'es, which need to be coerced back into floats, and code becomes unreadable
    ...
    // simple discrete-time highpass filter from Wikipedia
    ...
    // Copied from the original ActionScript source, haven't figured out how it works yet
    function resonate(heapStart, heapEnd) {
        ...
        // yeah, when accessing the heap with an index which is an expression, the total index expression is 
        // validated in a way that forces the index to be a byte and apparently '|0' coerces to signed when not in 
        ...
        for (i = heapStart << 2; (i|0) <= (heapEnd << 2); i = (i + 4)|0) {
            ...
            // I'm not sure why, but the resonating process plays havok with the DC offset - it jumps around everywhere.
    ...
    function fadeTails(heapStart, length) {
        ...
        var gain = 0.0;
        tailProportion = 0.1;
        // first convert length from an int to an unsigned (>>>0) so that we can convert it a double for argument of floor()
        // then convert it to a double (+) then convert the double result of floor to a signed with ~~
        // http://asmjs.org/spec/latest/#binary-operators
        ...
        // the result of an additive addition is an intish, which must be coerced back to an int
        tailSamplesStart = (heapStart + length - tailSamples)|0;
        heapEnd = (heapStart + length)|0;
        // so remember, i represents a byte index, and the heap is a Float32Array (4 bytes)
    ...
    // "smoothing factor" parameter is coefficient used on the terms in the main lowpass filter in Karplus-Strong loop
    function renderKarplusStrong(seedNoiseStart,seedNoiseEnd,targetArrayStart,targetArrayEnd,sampleRate, hz, velocity,
        ...
        // (byte-addressed) index of the heap as a whole that we get noise samples from
        var heapNoiseIndexBytes = 0;
        // (Float32-addressed) index of the portion of the heap that we'll be writing to
        var targetIndex = 0;
        // (byte-addressed) index of the heap as a whole where we'll be writing
        var heapTargetIndexBytes = 0;
        // (byte-addressed) index of the heap as a whole of the start of the last period of samples
        var lastPeriodStartIndexBytes = 0;
        // (byte-addressed) index of heap as whole from where we'll take samples from last period, after having added skip 
        // from tension
        ...
        */
        ...
        for (targetIndex = 0; (targetIndex|0) < (sampleCount|0); targetIndex = (targetIndex + 1)|0) {
            ...
            if ((targetIndex|0) < (periodSamples|0)) {
                ...
                // if stringTension == 1.0, we'd read from same sample we were writing to ordinarily, this would mean
                // that only first period of noise was preserved, and rest of buffer would be silence, but because 
                // we're reusing the heap, we'd actually be reading samples from old waves
                curInputSample = 0.0;
            }
            // the current period is generated by applying a low-pass filter to the last period
    ...
    function renderDecayedSine(targetArrayStart,targetArrayEnd,sampleRate, hz, velocity,decayFactor) {
        ...
        for (targetIndex = 0;
            ...
            heap[heapTargetIndexBytes >> 2] = velocity * pow(2.0, -decayFactor*time) *
                sin(2.0 * pi * hz * time);
        }
    }
    return { renderKarplusStrong: renderKarplusStrong, renderDecayedSine: renderDecayedSine, 
             fadeTails: fadeTails, resonate: resonate, };
}
